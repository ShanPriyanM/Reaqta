...
#Creation of application segment
...
#Creating internet gateway for servers to be connected to internet
...
#Added default route in routing table to point to Internet Gateway
...
#Creating private key which to login to the webserver
...
#Creating block volume for data persistence
...
#Attachng volume to created instance
...
#Mounting volume to my instance
resource "null_resource" "nullmount" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo git clone https://github.com/ShanPriyanM/Reaqta.git  /var/www/html"
    ]
  }
}
...
#Creating bucket to upload your static data like images
resource "aws_s3_bucket" "reaqtademo12345" {
  bucket = "reaqtademo12345"
  ...
  tags = {
    Name        = "reaqtademo12345"
    Environment = "Test"
  }

  provisioner "local-exec" {
    command = "git clone https://github.com/ShanPriyanM/Reaqta.git web-server-image"
  }

}

#Added public access to bucket
resource "aws_s3_bucket_public_access_block" "public_storage" {
  depends_on          = [aws_s3_bucket.reaqtademo12345]
  bucket              = "reaqtademo12345"
...
resource "aws_s3_bucket_object" "Object1" {
  ...
  acl        = "public-read-write"
  key        = "Reaqta.PNG"
  source     = "web-server-image/Reaqta.PNG"
}
...
resource "aws_cloudfront_distribution" "tera-cloufront1" {
  ...
  origin {
    domain_name = aws_s3_bucket.reaqtademo12345.bucket_regional_domain_name
...
#Updating CDN image URL to webserver code.
resource "null_resource" "Write_Image" {
  ...
  provisioner "remote-exec" {
    inline = [
      "sudo su << EOF",
      "echo \"<img src='http://${aws_cloudfront_distribution.tera-cloufront1.domain_name}/${aws_s3_bucket_object.Object1.key}' width='300' height='380'>\" >>/var/www/html/Reaqta.html",
      "echo \"</body>\" >>/var/www/html/Reaqta.html",
      "echo \"</html>\" >>/var/www/html/ReaqtaAPI.html",
      "EOF",
    ]
  }
...
resource "null_resource" "result" {
  ...
  provisioner "local-exec" {
    command = "echo The page has been displayed successfully and >> result.txt  && echo the IP of the website is  ${aws_instance.Web[0].public_ip} >>result.txt"
  }
}
